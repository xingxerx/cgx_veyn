...
- [x] 🔴 Build a **State Reduction Layer** inside `veyn-core` — sits between the raw event bus and `/context/current`
...
- [x] 🔴 Build a **Semantic Synthesis Engine** — translates filtered events into human-readable intent strings
...
- [x] 🟡 Add a **context snapshot ring buffer** in `veyn-core` — keep last N semantic snapshots (configurable, default 32)
- [x] 🟡 Expose `/context/history?n=10` endpoint returning last N snapshots for agent catch-up
- [x] 🟡 Add a `compression_ratio` field to the health endpoint so operators can observe how aggressively the firehose is being filtered
...
- [x] 🔴 Implement **token-based local auth**
...
- [x] 🔴 Implement **strict CORS policy**
...
- [x] 🔴 Add **WebSocket origin validation** — explicitly check `Origin` header on upgrade; reject non-localhost origins unless in allowlist
...
- [x] 🟡 Add **audit log** — every auth failure, new connection, and token generation written to `~/.local/share/veyn/audit.log`
- [x] 🟡 Strip raw keystroke content from `/context/current` by default — only expose semantic intent, not literal keystrokes
...
- [x] 🔴 Define a **VEYN Host ABI** — a stable, versioned set of WASI-style host functions that plugins call
...
- [x] 🔴 Write the WASM runtime host in `veyn-core` using `wasmtime` crate with explicit capability grants per plugin
- [x] 🟡 Add **plugin manifest format** (`plugin.toml`) — name, version, author, required capabilities, device descriptors
- [ ] 🟡 Add **plugin signature verification** — each plugin WASM binary must be signed; core validates on load
- [x] 🟡 Write a `veyn-sdk` crate with Rust macros that generate the ABI bindings so plugin authors don't write raw FFI
...
- [x] 🔴 Implement graceful shutdown on SIGTERM/SIGINT — drain event bus, flush state, close device handles cleanly
- [x] 🔴 Add `veyn.toml` config file with full schema documentation in comments
  - Port, auth settings, CORS origins, device class enables/disables, compression thresholds, log level
- [x] 🔴 Add `--config <path>` and `--port <port>` CLI flags
- [x] 🟡 Add structured logging (use `tracing` crate) with log levels: ERROR, WARN, INFO, DEBUG, TRACE
- [x] 🟡 Add a `/health` REST endpoint returning `{ status, uptime_s, connected_devices, event_rate_hz, compression_ratio }`
- [x] 🟡 Add a `/devices` REST endpoint listing all currently connected devices with type, ID, and status
- [ ] 🟡 Implement device hot-plug/unplug detection — daemon must not crash when a device disconnects mid-session
- [x] 🟡 Add event bus backpressure handling — if consumers are slow, drop oldest events rather than OOM-ing
...
- [x] 🟡 BLE adapter (cross-platform via `btleplug` crate) — scan, connect, notify subscriptions
...
- [x] 🔴 Define and document the canonical JSON schema for the context snapshot
  ...
  ```
- [x] 🔴 Version the API — all endpoints prefixed `/v1/`
- [ ] 🟡 Add OpenAPI 3.0 spec (`openapi.yaml`) — auto-generate or hand-write, keep in sync
- [x] 🟡 WebSocket stream: add a `ping/pong` keepalive with configurable interval
...
- [x] 🔴 Publish `veyn-sdk` as a proper Rust crate with `cargo add veyn-sdk` support
...
- [x] Add `cargo xtask` or `Makefile` with: `build`, `test`, `run-dev`, `lint`, `fmt`, `package`
- [ ] Add `docker-compose.yml` for a local dev stack (daemon + example consumer agent)
- [x] Add GitHub Actions CI: lint + test on push for Linux/macOS/Windows
...
|-|-----------------------------|------|-----------------------------------|
|1|Workspace `Cargo.toml`       |✅ Done|Repo won't build at all            |
|2|Token auth on `:7700`        |✅ Done|Active security vulnerability      |
|3|CORS + Host header validation|✅ Done|Browser-based keylogger attack     |
|4|Semantic compression layer   |✅ Done|AI context endpoint is unusable raw|
|5|WASM Host ABI                |✅ Done|Plugin system is non-functional    |
|6|Graceful shutdown            |✅ Done|Data loss / device handle leaks    |
|7|`veyn.toml` config schema    |✅ Done|No way to configure the daemon     |
|8|`README.md` + `INSTALL.md`   |✅ Done|Nobody can run it                  |
