...
- [x] **11.1 Desktop shell (`intero/`)**: Scaffold Tauri app (Rust backend + WebView). Auto-launch VEYN daemon. Secure local token handoff. System tray intent indicators.
- [x] **11.2 Live biometric dashboard**: SSE subscription to `/v1/context/subscribe`. Render `intent_code`, confidence, per-metric baseline z-scores, and scrolling signal strip charts.
- [x] **11.3 Decision session UX**: Start/stop session flow with decision annotations. In-session live multi-channel strips. Session replay with intent timeline overlay and flat CSV exports.
- [x] **11.4 Somatic feedback layer**: Approach/Avoidance indicator. Confidence-gated display (suppress UI when `confidence < 0.4`). Historical pattern panel matching past memory topics.
- [x] **11.5 Ambient background mode**: Background daily digests (HRV trends vs baseline). Configurable Fatigue/Recovery nudges.
...
- [x] **12.1 The Somatic CLI (Interpreter)**: Build a native terminal emulator/REPL in the TypeScript or Rust SDK that wraps standard shell executions. Hook into `/v1/context/current`.
- [x] **12.2 Biometric Command Gating**: Implement interception logic. Block high-risk commands (e.g., `git push`, `cargo publish`, `rm -rf`) if the active z-scores trigger `Fatigue` or acute `StressResponse` thresholds, demanding a biological or manual override.
- [x] **12.3 Biological Process Scheduler (Biometric `cron`)**: Implement a background task manager. Defer heavy background compilations, autonomous code generations, or system updates until the operator's context history ring buffer stabilizes into an `Approach` or `Recovery` vector.
- [x] **12.4 Dynamic Input Micro-Jitter Adaptation**: Map `evdev`/`hidraw` keyboard debounce timings dynamically. When `CognitiveLoad` spikes, automatically increase mechanical debounce limits to filter stress-induced mis-types, and shift shell autocomplete rankings to prioritize low-complexity syntax.
...
- [x] **15.1 MQTT rules engine**: `[mqtt_output]` block in `rules.toml` mapping `intent_code` transitions to MQTT topic/payload pairs (e.g., `StressResponse` -> scene="calm"). Add rule debounce timers.
- [x] **15.2 Multimedia Sinks**: Compile the `cpal` audio adapter for ambient RMS/peak ingestion. Develop the OSC output adapter to push live somatic z-scores down to DAW/VJ software.
...
- [x] **16.3 Research notebooks**: Jupyter files for HRV longitudinal mapping (`hrv_longitudinal.ipynb`) and session retrospective overlays.
...
- [x] **17.1 Mutual TLS (mTLS)**: Enforce Zero-Trust across multi-machine clusters via signed certificates in `veyn.toml`.
- [x] **17.2 Plugin registry & WASM limits**: `registry.toml` schema for third-party `.wasm` modules. Cap memory/CPU time via `wasmtime::Config`. 
- [x] **17.3 Infrastructure upgrades**: Structured `json` logs. Expanded Prometheus metrics (`GET /metrics`). Cross-platform GitHub Actions CI matrices. Go Language SDK (`sdk/go/`).
